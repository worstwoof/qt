...
practiclesystem::practiclesystem(int effect_duration)
    : particle_effect_duration(effect_duration*1.5),
      enabled_(false),
      num_rings(5),
      particles_per_ring(12),
      ring_initial_radius_step(13.0f),
      ball_initial_radius(2.1f),
      ball_max_radius_factor(2.0f),
      ring_expansion_rate(1.0f),
...
void practiclesystem::generateParticles(const cv::Point& effect_center) {
    for (int r_idx = 0; r_idx < num_rings; ++r_idx) {
        float first_ring_radius = 10.0f;
        float additional_radius_per_ring = ring_initial_radius_step;
        float current_ring_initial_radius = first_ring_radius + (r_idx * additional_radius_per_ring);
        for (int p_idx = 0; p_idx < particles_per_ring; ++p_idx) {
            ...
            p.current_ring_radius_from_center = current_ring_initial_radius;
