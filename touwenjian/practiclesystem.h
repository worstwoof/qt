...
public:
    ...
    void setEnabled(bool enabled);
     ...
     bool enabled_; // ✨ 新增：控制粒子特效是否开启的标志
      practiclesystem(int effect_duration);
private:
           std::vector<Particle> activeParticles; // 存储所有当前活跃的粒子

           int particle_effect_duration; // 特效持续的总帧数 (比如等于 processInterval)
           ...
           // 随机数生成器 (用于颜色)
           std::mt19937 rng_color_gen; // Mersenne Twister 随机数引擎
           std::uniform_int_distribution<int> dist_color_val; // 均匀分布0-255
};
